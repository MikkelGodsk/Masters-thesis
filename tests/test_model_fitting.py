...
from tests.cache_dir import cache_dir, OUTPUT_DIR
...
def overfit_to_small_batch(
    ...
    factory = Factory.spawn_factory(
        model_name, cache_dir=cache_dir, model_name=mock_model_name
    )
...
def test_overfit_to_small_batch_opt():
    checkpoint_dir = os.path.join(
        OUTPUT_DIR, "checkpoints", "test_overfit_to_small_batch_opt"
    )
    ...
    train_loss = overfit_to_small_batch(
        model_name, dataset_name, mock_model_name, checkpoint_dir
    )
    print(train_loss)
...
def test_overfit_to_small_batch_opt_backprop_trick():
    checkpoint_dir = os.path.join(
        OUTPUT_DIR, "checkpoints", "test_overfit_to_small_batch_opt_backprop_trick"
    )
    model_name: str = "facebook/opt-125m"
    mock_model_name: str = model_name  #'hf-internal-testing/tiny-random-OPTForCausalLM'   # Mock model doesn't seem to work for training - Has another embedding dimension...
    dataset_name: str = "GAIR/lima"
    train_loss = overfit_to_small_batch(
        model_name, dataset_name, mock_model_name, checkpoint_dir, backprop_trick=True
    )
    print(train_loss)
    assert torch.isclose(torch.tensor(train_loss), torch.tensor(0.0), atol=1e-2)
