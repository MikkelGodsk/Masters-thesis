...
def consistency_check(
    net1: nn.Module, net2: nn.Module, 
    optimzer1: torch.optim.Optimizer, optimizer2: torch.optim.Optimizer, 
    shape_in: List[int], shape_out: List[int]):
  """
  ...
  for j in range(100):
    ...
    assert not torch.all(p_init == next(iter(net1.parameters()))), "The network did not train"
    assert not torch.all(p_init == next(iter(net2.parameters()))), "The network did not train"
