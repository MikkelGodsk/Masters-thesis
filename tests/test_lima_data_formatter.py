...
correct_llama_train_instruction = "<s><s> [INST] Can brain cells move? By movement I mean long distance migration (preferably within the brain only). [/INST]"
...
Post-mitotic, but non-differentiated neurons have been shown to migrate in the adult brain in fish (Scott et al., 2012), and in mammals and non-human primates as well (Sawada et al., 2011).
Not surprisingly, glial cells, stem cells and neurons also migrate during embryonic development. Most notably, post-mitotic neurons destined to fulfill peripheral functions have to migrate over relatively long distances from the neural crest to their target locations (Neuroscience, 2nd ed, Neuronal Migration). </s>"""
correct_opt_test_instruction = "</s></s>I'm writing a NeurIPS paper about a new model architecture for processing and generating long texts. Here are some facts about the paper:\n* The main trick is to replace some of the attention heads with an exponential moving average, where the decay rate is learned for each head. We call this architecture ExeMA.\n* On language modeling, the perplexity difference between our model and a vanilla transformer is negligible, but that's because next-token prediction is almost always a local task, so perplexity won't be sensitive enough to detect any improvements in long-range understanding.\n* However, on the SCROLLS benchmark, our model improves by 10% over the baseline.\n* We also have a new metric for measuring coherence in generated text (CoGnaTe), where our model generates text that is 43% more coherent than the baseline.\nHelp me write the paper's introduction. <SEP>"
...
def test_data_formatter_exact_match():
    ...
    llama_train_instruction, llama_train_response, llama_tokenized_train_instruction, llama_tokenized_train_response = llama_formatter.get_instruction_and_response(
        llama_formatter.train_ds[0]
    )
    opt_train_instruction, opt_train_response, opt_tokenized_train_instruction, opt_tokenized_train_response = opt_formatter.get_instruction_and_response(
        opt_formatter.train_ds[0]
    )
    ...
    llama_test_instruction, llama_test_response, llama_tokenized_test_instruction, _ = llama_formatter.get_instruction_and_response(
        llama_formatter.test_ds[0]
    )
    opt_test_instruction, opt_test_response, opt_tokenized_test_instruction, _ = opt_formatter.get_instruction_and_response(
        opt_formatter.test_ds[0]
    )
    ...
    # Check cleaning of the instruction
    #assert (tokenizer_llama.eos_token_id in llama_tokenized_train_instruction)
    #assert (tokenizer_opt.sep_token_id in opt_tokenized_train_instruction) or (tokenizer_opt.eos_token_id in opt_tokenized_train_instruction)

    #llama_tokenized_train_instruction = llama_formatter.clean_tokenized_prompt(llama_tokenized_train_instruction)
    #opt_tokenized_train_instruction = opt_formatter.clean_tokenized_prompt(opt_tokenized_train_instruction)

    #assert (tokenizer_llama.eos_token_id not in llama_tokenized_train_instruction)
    #assert (tokenizer_opt.sep_token_id not in opt_tokenized_train_instruction) and (tokenizer_opt.eos_token_id not in opt_tokenized_train_instruction)
    #assert (tokenizer_llama.eos_token_id not in llama_tokenized_test_instruction)
    #assert (tokenizer_opt.sep_token_id not in opt_tokenized_test_instruction) and (tokenizer_opt.eos_token_id not in opt_tokenized_test_instruction)
