...
def get_train_dataloader_and_tokenizer(model_name:str, dataset_name:str, checkpoint_dir:str, mock_model_name:str|None = None):
    # Setup model and tokenizer
    factory = Factory.spawn_factory(architecture_type=model_name, cache_dir=cache_dir)  # For tokenizer, and maybe model
    if mock_model_name is not None:
        mock_factory = Factory.spawn_factory(architecture_type=model_name, model_name=mock_model_name, cache_dir=cache_dir)
    ...
    for batch in dataloader:
        for tokenized_instruction in batch['input_ids']:
            ...
            for token in tokenized_instruction:
                if (token in [tokenizer.bos_token_id, tokenizer.sep_token_id, tokenizer.eos_token_id]) and (token is not None):
...
def test_lima_train_dataloader_for_opt():
    ...
    checkpoint_dir = os.path.join(OUTPUT_DIR, 'checkpoints', 'test_lima_train_dataloader_for_opt')
    train_dataloader, tokenizer = get_train_dataloader_and_tokenizer(model_name, dataset_name, checkpoint_dir, mock_model_name=mock_model_name)
...
def test_lima_train_dataloader_for_llama2():
    ...
    dataset_name:str="GAIR/lima"
    checkpoint_dir = os.path.join(OUTPUT_DIR, 'checkpoints', 'test_lima_train_dataloader_for_llama2')
    train_dataloader, tokenizer = get_train_dataloader_and_tokenizer(model_name, dataset_name, checkpoint_dir, mock_model_name=mock_model_name)
    check_special_tokens(train_dataloader, tokenizer)
